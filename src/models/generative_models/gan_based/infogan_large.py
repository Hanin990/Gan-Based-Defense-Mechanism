# Derived from "Textual Manifold-based Defense Against Natural Language Adversarial Examples".
# This component is included for reference and was not evaluated in our experiments. 
...
class Generator(nn.Module):
    def __init__(self, latent_dim=100, code_dim=10):
        ...
        self.gen = nn.Sequential(
            ...
            nn.ConvTranspose1d(512, 256, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(256),
            nn.ReLU(inplace=True),
            # current size: bs x 256 x 64
            nn.ConvTranspose1d(256, 128, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
            # current size: bs x 128 x 128
            nn.ConvTranspose1d(128, 64, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            # current size: bs x 64 x 256
            nn.ConvTranspose1d(64, 32, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(32),
            nn.ReLU(inplace=True),
            # current size: bs x 32 x 512
            nn.ConvTranspose1d(32, 1, 4, stride=2, padding=1, bias=False),
            nn.Tanh(),
            # current size: bs x 1 x 1024
        )
    ...
    def forward(self, z, c, make_one_hot=True):
        ...
        output = self.gen(inp)  # (bs, 1, 1024)
        return output.squeeze(dim=1)  # (bs, 1024)
...
class Discriminator(nn.Module):
    def __init__(self, code_dim=10):
        ...
        self.dis = nn.Sequential(
            # input size: bs x 1 x 1024
            nn.Conv1d(1, 32, 4, stride=2, padding=1, bias=False),
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # current size: bs x 256 x 64
            nn.Conv1d(256, 512, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(512),
            nn.LeakyReLU(0.2, inplace=True),
            # current size: bs x 512 x 32
            nn.Conv1d(512, 1024, 4, stride=2, padding=1, bias=False),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(0.2, inplace=True),
            # current size: bs x 1024 x 16
            #nn.Conv1d(1024, 1, 16, stride=1, padding=0, bias=False),
            #nn.Sigmoid()
            # current size: bs x 1 x 1
        )

        self.dis_head = nn.Conv1d(1024, 1, 16, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        # input size: bs x 1024
        x = x.unsqueeze(dim=1)  # (bs, 1, 1024)
        reps = self.dis(x)  # (bs, 1024, 16)
