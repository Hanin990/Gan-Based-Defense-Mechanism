...
class ClassificationMetric(Metric):
    ...
    def get_metric(self, reset: bool = False) -> Dict:
        ...
        if total_num == 0:  # Check to avoid ZeroDivisionError
            logging.warning("No valid samples for evaluation. Returning default metrics.")
            return {'loss': float('nan'), 'accuracy': float('nan'), 'f1': float('nan')}
