...
class GensimWordEmbedding(AbstractWordEmbedding):
    ...
    def __init__(self, keyed_vectors_or_path):
        ...
        if isinstance(keyed_vectors_or_path, str):
            if keyed_vectors_or_path.endswith(".bin"):
                ...
                #     keyed_vectors_or_path, binary=True, encoding='utf-8', unicode_errors='ignore'
                # )
                # Load the FastText model
                self.model = FT_gensim.load_fasttext_format(keyed_vectors_or_path)
                # self.model = fasttext.util.reduce_model(self.model, 100)
    ...
    def __getitem__(self, index):
        ...
        try:
            if isinstance(index, str):
                return self.keyed_vectors[index]
            else:
                return self.keyed_vectors[self.keyed_vectors.index_to_key[index]]
        except KeyError:
            return None
        except IndexError:
            return None
...
#                 raise KeyError(index)
    ...
    def nearest_neighbours(self, index, topn, return_words=True):
        if isinstance(index, str):
            index = self.word2index(index)  # Convert word to index
        word = self.keyed_vectors.index_to_key[index]
        neighbours = self.keyed_vectors.most_similar(word, topn=topn)
        if return_words:
            return [word for word, _ in neighbours]
        else:
            return [self.keyed_vectors.key_to_index[word] for word, _ in neighbours]
        # word = self.keyed_vectors.index_to_key[index]
        # neighbours = self.keyed_vectors.most_similar(word, topn=topn)
        # if return_words:
        #     return [word for word, _ in neighbours]
